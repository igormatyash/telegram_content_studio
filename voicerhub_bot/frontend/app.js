...
const uiTextEn = {
  ...
  "Створити чернетку": "Create draft",
  "Зображення": "Image",
  "Що таке рубрика?": "What is a rubric?",
  "Рубрика — це постійний напрям контенту. Чим конкретніше ви опишете тему, ціль, тон і правила, тим менше AI буде повторюватися та тим точніше підбиратиме ідеї.": "A rubric is a recurring content direction. The more clearly you describe the topic, goal, tone and rules, the less AI will repeat itself and the more accurate the ideas will be.",
  "Активна рубрика": "Active rubric",
  "Банк тем": "Topic bank",
  "Ідеї — це ще не публікації": "Ideas are not posts yet",
  "Тут ви збираєте потенційні теми, відбираєте найсильніші й вирішуєте, що з ними робити далі: додати в контент-план або одразу створити чернетку.": "Collect potential topics here, choose the strongest ones, then decide what happens next: add them to the content plan or create a draft right away.",
  "Редакційний план": "Editorial plan",
  "Контент-план — це що і коли виходить": "The content plan is what goes out and when",
  "Сюди потрапляють тільки обрані теми з датою або AI-план на тиждень чи місяць. Це не банк ідей, а робочий розклад майбутніх публікацій.": "Only selected topics with dates or an AI-generated weekly/monthly plan appear here. This is not an idea bank, it is the working schedule for future posts.",
  "Додати до плану": "Add to plan",
  "У контент-плані": "In content plan",
  "Банк ідей": "Idea bank",
  "Наступний крок": "Next step",
  "Обрати дату для плану": "Choose a date for the plan",
...
function rubricDisplayName(slugOrName) {
  const rubric = (state.data?.rubrics || []).find(item => item.slug === slugOrName);
  const label = String(rubric?.name || slugOrName || "—");
  return label.replace(/\s*\([^)]*\)\s*/g, " ").replace(/\s*[:·]\s*.*$/g, "").trim() || label;
}
...
function renderIdeas() {
  ...
  document.querySelectorAll("[data-plan-idea]").forEach(button => button.onclick = () => openPlanIdeaForm(Number(button.dataset.planIdea)));
...
function openPlanIdeaForm(ideaId) {
  showForm(
    "Додати до контент-плану",
    `<div class="wide callout info"><strong>${translateText("Коли ця тема має вийти?")}</strong><p>${translateText("Після вибору дати ідея з’явиться в розділі «Контент-план». Там її можна буде перетворити на чернетку або перенести на інший день.")}</p></div><label class="wide">${translateText("Обрати дату для плану")}<input name="planned_for" type="date" required value="${localDateKey(new Date())}"></label>`,
    async form => {
      await api(`api/ideas/${ideaId}/plan`, {
        method: "POST",
        body: JSON.stringify({planned_for: form.get("planned_for")}),
      });
      delete state.lists.ideas;
      delete state.lists.plan;
      toast("Ідею додано до контент-плану");
      await refresh(true);
      renderIdeas();
    },
    {submitLabel:"Додати до плану", loadingLabel:"Додаємо…"},
...
function renderPlan() {
  ...
  target.innerHTML=`${bulkBar("plan",[["delete","Видалити","danger"]])}${planned.length?`<div class="table-wrap plan-table-wrap"><table class="users-table content-table plan-table"><thead><tr><th><input type="checkbox" data-select-all="plan" aria-label="Обрати всі пункти плану"></th><th>Дата</th><th>Тема</th><th>Рубрика</th><th>Етап</th><th>Наступний крок</th></tr></thead><tbody>${planned.map(item=>{
  ...
  target.querySelectorAll("[data-open-draft]").forEach(button => button.onclick = () => openEditor(Number(button.dataset.openDraft)));
}
...
function rubricFields(item={}) {
  return `<div class="wide rubric-form-intro"><strong>Що таке рубрика?</strong><p>Рубрика — це постійний напрям контенту. Чим конкретніше ви опишете тему, ціль, тон і правила, тим менше AI буде повторюватися та тим точніше підбиратиме ідеї.</p></div><label>Назва рубрики<input name="name" required value="${esc(item.name||"")}" placeholder="Наприклад: Поради клієнтам"><small class="field-help">Коротка назва, яку команда побачить у списках і фільтрах. Краще 1–3 слова без зайвих пояснень.</small></label><label>Шаблон<select name="template"><option value="">З нуля</option>${rubricTemplates.map(([name])=>`<option value="${esc(name)}">${esc(name)}</option>`).join("")}</select><small class="field-help">Можна обрати готову структуру, а потім відредагувати її під ваш бізнес.</small></label><label class="wide">Опис<textarea name="description" minlength="20" required placeholder="Про що саме має бути ця рубрика, для кого вона і яку користь дає читачу.">${esc(item.description||"")}</textarea><small class="field-help">Опишіть тематику простими словами: які питання покриває рубрика, які приклади доречні, що читач має зрозуміти.</small></label><label>Ціль рубрики<input name="goal" value="${esc(item.goal||"")}" placeholder="Підвищити довіру / навчити / продати"><small class="field-help">Для чого потрібні ці пости: довіра, освіта, заявки, підтримка клієнтів, прогрів до покупки.</small></label><label>Тон комунікації<input name="tone" value="${esc(item.tone||"")}" placeholder="Експертний, простий, дружній"><small class="field-help">Як має звучати текст саме в цій рубриці: спокійно, експертно, практично, з гумором або без нього.</small></label><label class="wide">Приклад теми<input name="example_topic" value="${esc(item.example_topic||"")}" placeholder="5 питань, які варто поставити перед покупкою"><small class="field-help">Один приклад сильно допомагає AI зрозуміти формат майбутніх ідей.</small></label><label class="wide">Правила для AI<textarea name="instructions" placeholder="Не писати загальні поради. Давати конкретні чеклисти, приклади з практики та короткий висновок.">${esc(item.instructions||"")}</textarea><small class="field-help">Додайте обмеження й побажання: що обов’язково згадувати, чого уникати, яка структура поста бажана.</small></label><label class="check-label wide"><input name="active" type="checkbox" ${item.active===0?"":"checked"}> Активна рубрика<small class="field-help">Активні рубрики доступні в генерації ідей, контент-плану й фільтрах.</small></label>`;
}
